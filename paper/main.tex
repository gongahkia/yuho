...
% Two-column overflow control.  acmart's tight column width frequently
% causes long \texttt{...} identifiers (paths, function names) to
% overhang. The combination below tells TeX to (a) prefer extra inter-
% word space over line overflow, (b) allow underfull-h-box stretch up
% to 3em, (c) hyphenate inside \texttt{} via the inconsolata font's
% built-in hyphenation hooks. Fixes the bulk of the user-flagged
% overflow cases without per-line edits.
\setlength{\emergencystretch}{3em}
\tolerance=2000
\hbadness=10000
% Allow line breaks at slashes inside paths (e.g. paper/methodology/foo).
\AtBeginDocument{%
  \makeatletter
  \def\BreakableSlash{\nobreak\hskip\z@skip /\nobreak\hskip\z@skip}%
  \makeatother
...
\providecommand{\statBenchmarkFixtures}{205}%
  ...
  \providecommand{\statCaseLawMRR}{0.523}%

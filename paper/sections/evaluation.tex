...
ergonomics. The \texttt{throughput.json} file also records a
\texttt{min} of $-1$ days for one section, which indicates a
\texttt{last\_verified} stamp written before the matching first commit
in git history; we have not yet root-caused this and flag it as an
open measurement-pipeline bug. The defensible cost number is the
\emph{per-section agent compute time}, not the calendar lag, and we
report it next.

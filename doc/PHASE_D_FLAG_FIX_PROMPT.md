...
3. No fabrication rules (same as the original encoding prompt): if the statute says "with fine" without a number → `fine := unlimited`; if the statute says "liable to caning" without a number → do NOT add a stroke range, use a `supplementary := "…"` line quoting the canonical text; missing limbs must be quoted verbatim from `_raw/act.json`.
4. Some flags use the "three-way any combination of punishments" pattern (G12 in `doc/PHASE_C_GAPS.md`). The current workaround: two sibling penalty blocks — `penalty cumulative { imprisonment := …; supplementary := "imprisonment imposed"; }` + `penalty or_both { fine := unlimited; caning := 0 .. 0 strokes; supplementary := "fine or caning or both, per canonical text"; }`. If the canonical says "liable to caning" with no number, omit the caning clause and note the liability in supplementary text.

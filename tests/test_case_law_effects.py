...
def test_distinguished_case_law_effect_is_inactive() -> None:
    module = _module(
        """
        statute 1 "Theft" {
            elements { actus_reus taking := "takes"; }
            ...
            caselaw "New v PP" "[2026] SGCA 1" {
                "Old v PP is distinguished"
                element taking
                treatment distinguishes "Old v PP" "[1990] SGHC 1"
            }
        }
        """
    )
...
def test_followed_treatment_adopts_target_effect() -> None:
    module = _module(
        """
        statute 1 "Cheating" jurisdiction singapore {
            elements { actus_reus deception := "deception"; }
            ...
            /// @court_level apex
            /// @date 2026-01-01
            caselaw "Followed Adopter" "[2026] SGCA 1" {
                "Follows source"
                element deception
                treatment follows "Restrictive Source" "[2020] SGCA 1"
            }
    ...
    adopted = [
        case for case in effects["deception"]
        if case.case_name.value == "Followed Adopter"
    ]
    assert len(adopted) == 1
    assert adopted[0].interpretive_effect == "requires"
    assert adopted[0].effect_fact == "active_misleading"
...
def test_approved_treatment_adopts_target_effect() -> None:
    module = _module(
        """
        statute 1 "Cheating" jurisdiction singapore {
            elements { actus_reus deception := "deception"; }
            ...
            /// @court_level apex
            /// @date 2026-01-01
            caselaw "Approved Adopter" "[2026] SGCA 1" {
                "Approves source"
                element deception
                treatment approves "Restrictive Source" "[2020] SGCA 1"
            }
        }
        """
    )
    ...
    adopted = [
        case for case in effects["deception"]
        if case.case_name.value == "Approved Adopter"
    ]
    assert len(adopted) == 1
    assert adopted[0].interpretive_effect == "requires"
    assert adopted[0].effect_fact == "active_misleading"

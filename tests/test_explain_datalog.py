...
def test_datalog_explainer_applies_executable_case_law_effects():
    source = """
    statute 1 "Theft" {
    ...
    trace = DatalogExplainer().explain(
        statute,
        {"taking": True, "control_plus_deprivation": False},
    )

    assert trace.overall_satisfied is False
    assert trace.elements[0].satisfied is False
    assert "Case law 'New v PP' requires control_plus_deprivation" in trace.elements[0].reason

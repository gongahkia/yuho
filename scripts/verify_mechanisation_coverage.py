...
FEATURE_COVERAGE = (
    ...
    ("case_law_doctrine", "Yuho/CaseLaw.lean effect-surface-alias + treatment-surface-alias + surface-precedence-rank + vertical-stare-decisis binding/persuasive gate + binding-effect gate + fact-key + same-kind-nonconflict + effect + cumulative-effect + ordered-cumulative + negative-treatment-nonadoption + distinguished/overruled/reversed/disapproved-inactivation + own-effect + ordered-positive-adoption + followed-adoption + approved-adoption + applied-adoption + followed-approved-applied-noninactivation + missing-target + effectless-target + target-remap + payload-preserve + transitive/cyclic adoption + metadata override/fallback/merge + inactive-authority + jurisdiction-burden-metadata + precedence-rank/conflict lemmas", "partial"),

...
def main() -> int:
    ...
    # LaTeX control sequences are alpha-only — a digit ends the name
    # and breaks `\newcommand`. Spell numeric suffixes out (G4 → GFour,
    # G11 → GEleven, P95 → PNinetyFive) so the file `\input`s cleanly.
    tex_path.write_text(
        "% Auto-generated by scripts/paper_methodology.py — do not edit.\n"
        f"\\newcommand{{\\statFidelityGFour}}{{{fidelity['g4_illustration_count']['count']}}}\n"
        ...
        f"\\newcommand{{\\statFidelityGEleven}}{{{fidelity['g11_disjunctive_mismatch']['count']}}}\n"
        ...
        f"\\newcommand{{\\statThroughputPNinetyFive}}{{{throughput['encoding_to_stamp_days'].get('p95', 'TBD')}}}\n"
    )

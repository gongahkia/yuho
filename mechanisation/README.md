...
| Case-law executable effect fragment | `Yuho/CaseLaw.lean` | `CaseEffectKind.requires_false` + `satisfies_true` + `excludes_true` + `TreatmentKind.followed_adopts` + nil lemmas | Direct computation over the three runtime-supported effect operators plus one-hop positive treatment adoption; smoke tests cover `requires` on a targeted element and adopted effect transfer |
...
* **Richer doctrine and case-law semantics.** The current artefact
  mechanises the executable effect algebra and one-hop positive treatment

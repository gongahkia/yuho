...
def orderedPositiveAdopterCase : CaseAuthority :=
  { name := "Ordered Positive Adopter"
    element := "taking"
    effect := none
    burdenShift := none
    jurisdiction := none
    treatments := [(.followed, "Foreign Restrictive"),
      (.approved, "Foreign Expansive")]
    precedence := neutralPrecedence
  }
...
def normalizedFactConflictCase : CaseAuthority :=
  { name := "Normalized Fact Conflict"
    element := "taking"
    effect := some
      { takingRequiresControl with
        kind := .satisfies
        fact := "control-plus-deprivation"
      }
    burdenShift := none
    jurisdiction := none
    treatments := []

...
### Recommended near-term sequence `[x]`

All eight items shipped this session. Each artefact is self-contained
and has its own README; this list is a status board, not a how-to.

- [x] **G10 cross-section resolver.** `src/yuho/library/reference_graph.py` + `yuho refs` CLI + 19 tests. Live: 152 nodes / 130 edges (5 subsumes, 125 implicit).
- [x] **Penal Code explorer — three artefacts sharing one corpus.**
    - [x] **2a. Enriched JSON corpus.** `library/penal_code/_corpus/{index.json,sections/s*.json}` built via `scripts/build_corpus.py`. 524 records, 4.8 MB; per-section JSON has raw text, `.yh`, English transpilation, Mermaid, AST summary, L1/L2/L3 + flags, references in/out, SSO anchor, provenance hashes. 15 tests passing.
    - [x] **2b. Browser extension v0.** `editors/browser-yuho/`. Manifest V3, content script on `sso.agc.gov.sg/Act/PC1871*`, side panel with five tabs (Overview, English, Elements, References, .yh source), pop-up flag callouts, dark-mode aware. Bundle ~2.9 MB.
    - [x] **2c. Static explorer site.** `editors/explorer-site/`. Plain Python+stdlib generator (`build.py`); produces 524 per-section pages + index/coverage/flags/about. Client-side search via `static/search.js`. Deploy by copying `build/` to any static host.
- [x] **Yuho benchmark pack.** `benchmarks/`. Five task types — citation grounding, penalty extraction, element classification, cross-reference (G10), illustration recognition — totalling 3,319 rows. Scorer at `benchmarks/scorers/score.py` produces accuracy + mean F1 per task. Identity-check (gold = pred) yields 100% across all tasks.
- [x] **Scenario / fact-pattern simulator.** `simulator/`. YAML/JSON fact-pattern schema + minimal stdlib parser; matches encoded element names against `fact_facts` direct booleans plus prose substring overlap; reports satisfied / contradicted / suggested / unresolved + asserted-vs-recognised exceptions. Three worked fixtures (s415 classic, s415 with consent, s378 theft).
- [x] **Provenance and fidelity ledger.** `library/penal_code/_ledger/{ledger.json,LEDGER.md}`. Per-section: SHA-256 of canonical text, first/last git commit, L1/L2/L3 state with stamp dates, flag detail, contributor metadata. Generated by `scripts/build_ledger.py`.
- [x] **Grounded AI answer verifier.** `scripts/verify_grounded.py`. Takes a JSON answer document with explicit citations; verifies every citation against `raw` / `yh` / `english` corpus artefacts; reports orphan claims and spurious citations. Exit-non-zero on `--strict`. Smoke-tested with a grounded + an ungrounded answer.
- [x] **Paper methodology hardening.** `scripts/repo_stats.py` emits `paper/stats-extra.tex` with reproducible SLOC counts per layer (~51k implementation + 23k library). The deeper methodology runs (fidelity hit-rate spot-check, encoding throughput numbers, gap-trigger bar chart) remain `\todo{}` in `paper/sections/evaluation.tex`; data sources are now scriptable.
- [x] **Practitioner-facing surfaces — Microsoft Word add-in.** `editors/word-yuho/`. Office Add-in manifest (Insert ribbon group + taskpane) with TypeScript-style JS taskpane. Three insert actions: English transpilation, formatted citation, bulleted element list. Reads same slim corpus as the browser extension.

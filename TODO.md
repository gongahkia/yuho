...
- Close the known Lean proof gaps incrementally.
  ...
  - Current support: Lean case-law effect algebra mechanises lower-case surface alias normalisation, surface court/role precedence ranking, the `requires`/`satisfies`/`excludes` executable fragment, non-conflicting cumulative effects, own-effect dominance over adoption, missing-target adoption skip, adopted-effect target remapping and fact/kind preservation, bounded positive-treatment-chain adoption with cycle cutoff and metadata merge, inactive-treatment suppression, jurisdiction-gated burden-metadata guards, concrete precedence ranks, and same-fact conflict selection.
  - Add richer doctrine/case-law semantics beyond the typed-fact/surface-alias/surface-precedence-rank/effect/cumulative-effect/own-effect/missing-target/target-remap/payload-preserve/adoption-cycle/adoption-metadata/inactivation/jurisdiction-burden-metadata/precedence-rank/conflict fragment.

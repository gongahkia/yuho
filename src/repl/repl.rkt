...
(define global-env (make-hasheq))

(define (error-message msg)
  (error (format "ERROR: ~a" msg)))
...
(define (parse-expr input)
  (with-input-from-string input
    (lambda ()
      (let ([tokens (tokenize (port->string (current-input-port)))])
        (parse tokens)))))

(define (eval-expression expr env)
  (cond
    [(number? expr) expr]
    [(symbol? expr) (hash-ref env expr (error 'eval-expression "undefined variable" expr))]
    [(list? expr)
     (let ([op (car expr)]
           [args (cdr expr)])
       (cond
         [(eq? op 'define)
          (let ([var (cadr expr)]
                [value (eval-expression (caddr expr) env)])
            (hash-set! env var value)
            value)]
         [(eq? op 'if)
          (let ([test (eval-expression (car args) env)]
                [then-branch (cadr args)]
                [else-branch (caddr args)])
            (if test
                (eval-expression then-branch env)
                (eval-expression else-branch env)))]
         [(symbol-list? (car expr))
          (let ([func (eval-expression (car expr) env)]
                [args (map (lambda (e) (eval-expression e env)) (cdr expr))])
            (apply func args))]
         [else (error 'eval-expression "unknown expression" expr)]))]
    [else (error 'eval-expression "unknown expression" expr)]))
...
(define (repl)
  (let loop ([env global-env])
    ...
    (unless (string=? input "exit")
      (let ([parse-result (parse-expr input)])
        (if (error? parse-result)
            (error-message "Failed to parse input")
            (begin
              (check-for-errors parse-result)
              (check-for-reminders parse-result)
              (let ([results (map (lambda (stmt) 
                                    (eval-expression (evaluate-statement stmt) env))
                                  parse-result)])
                (for ([result results])
                  (displayln result))))))
      (loop env))))

(define (evaluate-statement stmt)
  (match stmt
    [(declaration _ _ expr) expr]
    [(assignment id expr) (list 'define id expr)]
    [(function-call id args) (list id args)]
    [(match-case expr cases) (list 'match expr cases)]
    [(pass-statement) '()]
    [_ (error 'evaluate-statement "unknown statement" stmt)]))

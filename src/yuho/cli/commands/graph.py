...
class DependencyExtractor(Visitor):
    ...
    def extract(self, ast: ModuleNode, filename: str = "main") -> DependencyGraph:
        ...
        self.graph.add_node(GraphNode(id=module_id, label=filename, node_type="module"))
        ...
        for type_def in ast.type_defs:
            ...
            self.graph.add_node(GraphNode(id=type_id, label=type_def.name, node_type="type"))
            self.graph.add_edge(GraphEdge(source=module_id, target=type_id, edge_type="defines"))

        # Process functions
        for func in ast.function_defs:
            func_id = f"function:{func.name}"
            self.graph.add_node(GraphNode(id=func_id, label=func.name, node_type="function"))
            self.graph.add_edge(GraphEdge(source=module_id, target=func_id, edge_type="defines"))
    ...
    def _process_import(self, module_id: str, imp: ImportNode) -> None:
        ...
        self.graph.add_node(GraphNode(id=import_id, label=imp.path, node_type="import"))
        ...
        self.graph.add_edge(
            GraphEdge(source=module_id, target=import_id, edge_type="imports", label=label)
        )
    ...
    def _process_statute(self, module_id: str, statute: StatuteNode) -> None:
        ...
        self.graph.add_edge(GraphEdge(source=module_id, target=statute_id, edge_type="contains"))
    ...
    def _extract_section_refs(self, source_id: str, text: str) -> None:
        ...
        for pattern in patterns:
            for match in re.finditer(pattern, text):
                ...
                self.graph.add_node(
                    GraphNode(id=ref_id, label=f"S.{ref_section}", node_type="statute")
                )

                # Add reference edge
                self.graph.add_edge(
                    GraphEdge(source=source_id, target=ref_id, edge_type="references")
                )
...
def run_graph(
    ...
    if result.root_node is None:
        click.echo(colorize(f"error: No parse tree available for {file}", Colors.RED), err=True)
        sys.exit(1)

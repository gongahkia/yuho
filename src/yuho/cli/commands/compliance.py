    ...
    for statute in result.ast.statutes:
        ...
        for elem in statute.elements:
            if isinstance(elem, nodes.ElementGroupNode):
                _flatten_group(elem, requirements)
                ...
                requirements.append(
                    {
                        ...
                        "description": (
                            exc.condition.value
                            if isinstance(exc.condition, nodes.StringLit)
                            else "exception condition"
                        ),
                        "compliant": None,
                    }
                )
...
def _flatten_group(group: nodes.ElementGroupNode, reqs: List[Requirement]) -> None:
...
def _render_markdown(matrix: List[ComplianceEntry]) -> List[str]:

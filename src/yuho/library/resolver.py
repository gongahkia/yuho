...
class DependencyResolver:
    ...
    def add_available_package(self, package: PackageInfo) -> None:
        ...
        self._available[package.section_number].sort(key=lambda p: p.version, reverse=True)
        ...
        while to_resolve:
            ...
            if package in resolved:
                ...
                if not self._check_constraints(package, resolved[package]):
                    errors.append(f"Conflict: {package}@{resolved[package]} violates constraints")
                continue
...
class CompatibilityChecker:
    ...
    def check_constraint_compatibility(
        ...
        if constraint1.op == ConstraintOp.EQ:
            if constraint1.version is not None and constraint2.satisfies(constraint1.version):
                return (True, constraint1)
            return (False, None)
        if constraint2.op == ConstraintOp.EQ:
            if constraint2.version is not None and constraint1.satisfies(constraint2.version):
                return (True, constraint2)
            return (False, None)

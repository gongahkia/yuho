...
class AkomaNtosoTranspiler(TranspilerBase):
    ...
    def transpile(self, ast: nodes.ModuleNode) -> str:
        ...
        lines.append('        <TLCOrganization eId="yuho" href="/ontology/organization/yuho" showAs="Yuho DSL"/>')
        lines.append('        <TLCOrganization eId="parliament" href="/ontology/organization/sg.parliament" showAs="Parliament of Singapore"/>')
    ...
    def _emit_statute(self, lines: List[str], statute: nodes.StatuteNode) -> None:
        ...
        if statute.elements:
            ...
            for i, elem in enumerate(flat, 1):
                ...
                lines.append(f'            <content><p class="{etype}" data-name="{name}">{desc}</p></content>')

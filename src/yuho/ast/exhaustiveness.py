...
class PatternMatrix:
    ...
    def width(self) -> int:
        ...
        return len(self.rows[0].patterns) if self.rows else 0
    ...
    def specialize(self, col: int, constructor: AbstractPattern) -> "PatternMatrix":
        ...
        for row in self.rows:
            ...
            if pattern.is_wildcard():
                ...
                new_patterns = row.patterns[:col] + list(pattern.children) + row.patterns[col + 1 :]
                new_rows.append(PatternRow(new_patterns, row.arm_index))
...
class ExhaustivenessChecker(Visitor):
    ...
    def _check_usefulness(
        self, matrix: PatternMatrix, scrutinee_type: TypeAnnotation
    ) -> Optional[List[str]]:
        """
    ...
    def _get_column_constructors(
        self, matrix: PatternMatrix, col: int, scrutinee_type: TypeAnnotation
    ...
    def _all_constructors_present(
        self, matrix: PatternMatrix, col: int, constructors: List[AbstractPattern]
    ) -> bool:
...
def check_exhaustiveness(
    module: nodes.ModuleNode, type_info: Optional[TypeInferenceResult] = None

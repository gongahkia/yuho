...
Parity, determinism, diagnostics and packaging are mandatory. If both candidates pass, compare reviewed clarity and proof mapping, packaging, latency and RSS. A technical tie means both satisfy every mandatory gate and neither has a material advantage in any of those dimensions; **maintainer preference decides a genuine tie**. At spike start the maintainer leaned toward Haskell, which was legitimate maintainability evidence but could not change fixtures, implementation effort, measurements or score reporting. If neither passed, the boundary would have needed reconsideration. The planned two independent numerical clarity scores were not completed: one independent runtime scored both candidates, and the maintainer then selected Haskell as the maintainability decision without issuing a second numerical code-review score.

...
The first production/proof correspondence **target** is **ClosedBooleanBranches-v1**: finite closed named Boolean requirements, recursive All/Any, inherited subsection paths, sibling alternatives, a total Boolean fact map and definition-only failure. It excludes arbitrary expressions, exceptions, penalties, outcomes and external dependencies. The second bounded Haskell fragment is [**AcyclicGuardedExceptions-v1**](ACYCLIC-GUARDED-EXCEPTIONS-V1.md): registered targets in a complete acyclic graph, the same facts/date/policy, all guards evaluated in declaration order, branch-scoped defeat and typed rejection of invalid or unsupported models. The third, [**TypedBooleanFacts-v1**](TYPED-BOOLEAN-FACTS-V1.md), binds explicit Boolean records to exact executable leaf IDs, validates declared metadata across the full registry and rejects mismatch as `KINV007` rather than false. Its provenance labels are descriptive, not authenticated evidence. The fourth, [**GuardedPenaltySelection-v1**](GUARDED-PENALTY-SELECTION-V1.md), selects stable penalty declaration IDs with ordered root-branch supports, exact trace-based guards and path-specific `KSEL001` overlap warnings. The fifth, [**PenaltyTerms-v1**](PENALTY-TERMS-V1.md), validates complete-registry authored punishment-description trees and attaches them only to selected IDs; it does not choose a child or calculate a sentence. These fragments' proof-neutral vectors are test evidence, not correspondence proofs. Proof statuses, sentencing and outcomes remain separate future work. Singapore Penal Code section 84 with a Criminal Procedure Code disposition is a later product-discriminating slice: cross-Act packages and that CPC corpus are open work (#63/#66), outcomes are partial (#51), and present s84 Boolean-fact tests do not establish doctrine or procedure.
...
The first theorem target is a reference-level property for ClosedBooleanBranches-v1, such as deterministic branch results on well-formed closed input. A later refinement theorem must state how parsed KernelInput and executable code relate to a chosen proof model; Lean, Rocq and F* remain candidates. The exception, typed-fact, penalty-selection and penalty-term fragments have only closed-input validation, synthetic fixtures, bounded properties and proof-neutral vectors. No theorem or implementation-refinement claim follows from their tests.
...
Only the bounded Haskell [production foundation and its five named fragments](../../rewrite/haskell/README.md) are authorised now. The broader rewrite, grammar regeneration, corpus migration, v1.2 change and Python retirement remain out of scope.

...
This v0.2 subset supports one explicit synthetic offence/exception composition, not arbitrary graph linking, multi-exception policies, modules with executable cross-references, imports from remote registries, version ranges, real legal doctrine, dynamic burden shifting, general computation, evidence assessment or court outcomes. [YuhoSurface-v0.3](YUHO-SURFACE-V0.3.md) adds bounded synthetic date selection over complete v0.2 compositions without changing v0.2 source or request bytes. Future bundle versions require an explicit packaging and comparator decision before module sources or locks become core artifacts.

...
The maintainer accepted the existing [section 84 authority research](SINGAPORE-SECTION-84-AUTHORITY-AND-TEMPORAL-RESEARCH.md), [locked source intake](SINGAPORE-SECTION-84-SOURCE-INTAKE-REPORT.md) and [qualified-review record](../../research/singapore/section-84-pilot/QUALIFIED-REVIEW-RECORD.json) as sufficient to implement a bounded research prototype. This milestone uses those already reviewed materials; it does not make a new online source-acquisition check a prerequisite. The earlier reported written confirmation covers the research baseline `c057fb84000265b412d1b6ca3d050e7b2a1d5eca` and proposed structure, subject to documented limits. A separate [implementation-conformance record](../../research/singapore/section-84-pilot/IMPLEMENTATION-CONFORMANCE-RECORD.json) now records the reviewer’s written confirmation for the exact ModelBundle digest `02e51da9fa1bf285eec7cca8b55494c1d6a8e82b9b7e7e300a30b338ecbd404a` and implementation baseline `bd36402a1a9ba5cfef6ef36d1b0ba0dd999a3a74`. Both records are unsigned, the reviewer’s identity is withheld, and qualification is maintainer-attested. The new confirmation is not cryptographically authenticated and does not apply to any other bundle digest.
...
The baseline qualified review did not transfer to the executable digest. The separately recorded implementation-conformance confirmation applies only to the exact digest above and its documented research-prototype limitations. The [internal record](../../research/singapore/section-84-pilot/IMPLEMENTATION-CONFORMANCE-RECORD.json) cites hashes of implementation-baseline files separately from the ModelBundle digest; it is outside the bundle and does not alter that digest or add a ModelBundle-v1 review assertion. The immutable bundled scope still records the pending state at construction time; editing it now would create a different bundle digest. A changed bundle digest requires a new implementation-conformance review. Neither record authenticates the reviewer or adds an autonomous Singapore-law outcome.

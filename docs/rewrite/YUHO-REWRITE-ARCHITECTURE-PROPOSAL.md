...
**Status:** Repository-reconciled proposal; three bounded Haskell kernel fragments are implemented
...
The result contains overall status (true, false or unresolved where the fragment permits), branch statuses in source order, element results, inherited citation paths, essential trace edges, source references, ordered diagnostics and capability/fragment version. The exception-discriminated result adds branch defeat reasons and registered dependency observations. The [typed Boolean fragment](TYPED-BOOLEAN-FACTS-V1.md) adds ordered evaluated-leaf observations and whole-registry annotation checks while leaving proof status, selected penalty source IDs and typed outcomes for later work. Status values and proof status are separate; unresolved may never normalize to false. Diagnostic equality prioritizes code, stage, severity and span over human wording.
...
| Later | Guarded penalties | Branch inheritance, guarded sibling selection and YRTP001 overlap require a separate versioned fragment; no penalty meaning is inferred from Boolean fact metadata. | [test_runtime_penalties.py](../../tests/test_runtime_penalties.py); s304A rash/negligent fixtures are migration evidence, not reviewed doctrine. |
...
4. The bounded [AcyclicGuardedExceptions-v1](ACYCLIC-GUARDED-EXCEPTIONS-V1.md) and [TypedBooleanFacts-v1](TYPED-BOOLEAN-FACTS-V1.md) fragments now have separate schemas, fixtures and proof-neutral vectors. Guarded penalties and proof states remain later, separate extensions. Broaden to representative corpus strata only after small cases pass.

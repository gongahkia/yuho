...
| Runtime eval | partial | `scripts/verify_runtime_tests.py`, `tests/test_runtime_z3_differential.py`, `tests/test_runtime_subsections.py`, `tests/test_runtime_penalties.py` | Canonical subsection branches preserve citation paths and fail definition-only provisions closed. Penalty results retain selected source blocks, guards, and branch paths; overlapping sibling guards emit a diagnostic. Element truth still often comes from boolean fact keys. |
...
| Z3 | partial | `tests/test_soundness_sanity.py`, `tests/test_z3_apply_scope.py`, `make verify-runtime-tests` | Conformance-tested SMT backend; unsupported subsection branches, conditional/multiple penalty blocks, case-law, and typed-burden constructs are rejected explicitly by consistency checking. |

...
| Haskell Core Yuho v0.2 typed finite rules | stable-bounded | `foundation-test`, `make verify-core-yuho-conformance` | Additive nominal entities, typed predicates/scalars, explicit negation, finite quantifiers, cardinalities and explicit priority lower to `TypedFiniteRules-v1`. Twenty-five registered Lean properties and 2,518 independent Haskell–Lean vectors cover the documented finite operations; parser/lowering refinement and legal correctness are not proved. |
| Haskell Core Yuho v0.3 release extensions | stable-bounded | `foundation-test`, `make verify-core-yuho-v03-conformance` | Finite normative positions and actor responsibility routes elaborate to `TypedFiniteRules-v1`; death, life-imprisonment and bounded-caning candidate terms lower to `PenaltyTerms-v1`. Eleven registered Lean properties and 33 independent vectors cover the bounded definitions. |
| Haskell v1 CLI | stable-bounded | `foundation-test`, release verification | `check`, `compile`, `run`, `explain`, `diagram`, `corpus`, `doctor`, `init`, `version` and `release verify` form the supported research workflow. `fmt` is explicitly deferred because comments are not retained by the authoritative lexer. |
...
| Singapore Criminal Law Research Corpus v0.3 | stable-bounded | `yuho corpus check`, `foundation-test` (`CorpusChecks`) | Canonically indexes 524 saved Penal Code provision rows and distinguishes structural, definition-only, partial and executable coverage. The Haskell CLI queries the local artifact and emits bounded native reference graphs; indexing is not legal currency or legal review. |
| Legacy Python parser | partial-archival | `tests/test_*grammar*.py`, `make verify-coverage` | Tree-sitter remains compatibility evidence for the archival Python product. It is not the parser used by the Haskell v1 workflow. |
...
| Legacy Python formatter | partial-archival | `tests/test_fmt_cli.py` | Not the Haskell v1 formatter. Haskell `fmt` is intentionally absent until comments and the complete release grammar can round-trip safely. |
| Runtime eval | partial | `scripts/verify_runtime_tests.py`, `tests/test_runtime_z3_differential.py`, `tests/test_runtime_subsections.py`, `tests/test_runtime_penalties.py`, `tests/test_runtime_exception_dependencies.py` | Canonical subsection branches preserve citation paths and fail definition-only provisions closed. Registered, acyclic exception-guard dependencies use the active facts and statute registry, retain target sub-results, and diagnose missing/cyclic/unsupported guards as unresolved. Penalty results retain selected source blocks, guards, and branch paths; overlapping sibling guards emit a diagnostic. Element truth still often comes from boolean fact keys. |
...
| Lean | stable-bounded | `make verify-mechanisation`, the three Core conformance commands | Core Yuho v0.1-v0.3 have 79 registered machine-checked properties and bounded independent conformance. This is not an end-to-end compiler, corpus or legal-correctness proof. The older Python/Z3 structural mechanisation remains a separate historical boundary. |

...
| Runtime eval | partial | `scripts/verify_runtime_tests.py`, `tests/test_runtime_z3_differential.py`, `tests/test_runtime_subsections.py` | Canonical subsection branches preserve citation paths and fail definition-only provisions closed; element truth still often comes from boolean fact keys, and guarded/sibling penalties remain unsupported. |

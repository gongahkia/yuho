...
> Legacy Python 5.1 CLI reference. The stable Haskell v1 command surface is documented in the [research release guide](../rewrite/HASKELL-YUHO-RESEARCH-LANGUAGE-v1.0.md#stable-command-surface).

_Auto-generated by `scripts/gen_cli_reference.py` on 2026-09-04._
...
| `yuho ci-report` | Validate every .yh file below DIRECTORY and write one CI report. |
...
| `yuho verify` | Run retained verification backends. |
...
Options:
  --format [json|sarif]  Machine-readable report format  [default: sarif]
  -o, --output PATH      Report path  [required]
  --syntax-only          Skip semantic analysis
  --help                 Show this message and exit.
```
...
Commands:
  ...
  ci-report   Validate every .yh file below DIRECTORY and write one CI report.
  ...
  verify      Run retained verification backends.
```

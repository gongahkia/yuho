...
import Yuho.Surface.ActorExceptions
  ( scopeTree, sharedExceptionTree, attachmentRuleId, attachmentProgramId
  , attachmentPath, attachmentTargetToken )
...
lowerChecked (Checked model scenario assignments firstTree secondTree) = do
  ...
  request <- case modelBody model of
    ...
    ActorScopedLegal _ _ _ _ offence (IntentionalAidRoute route _) attempt
      shared attachments _ _ -> case (scenario,secondTree) of
        (Just (ActorScopedScenario _ _ [target] observed _ _ _ _ _ _ _ _),
          ...
          let AttemptDefinition attemptRuleId _ _ _ _ = attempt
              selectedRule

...
data CheckedCase = CheckedCase FilePath Token Model [CheckedIssue]

caseModelFile :: FilePath -> AnalysisCase -> Either Diagnostic FilePath
caseModelFile path (AnalysisCase _ location _ _) =
...
checkAnalysisCase casePath declaration@(AnalysisCase caseId _ bindings allegations)
  modelPath model = do
  ...
  if length allegations == 3 &&
      Set.fromList [kind | CaseAllegation _ kind _ _ _ <- allegations] ==
  ...
  let ids = [tokenText item | CaseAllegation item _ _ _ _ <- allegations]
  ...
  let ordered = [item | kind <- [CaseOffence,CaseParticipation,CaseAttempt],
        item@(CaseAllegation _ actual _ _ _) <- allegations, actual == kind]
  issues <- mapM (checkIssue offence participation attempt) ordered
  pure (CheckedCase casePath caseId model issues)
  where
    checkIssue offence participation attempt allegation@(CaseAllegation item kind target role raw) = do
...
issueValue (CheckedIssue (CaseAllegation item kind target _ _) _ _) =
  (kind,item,target)
...
requestValue path (CheckedIssue (CaseAllegation item _ _ _ _) _ bytes) =
...
compileAnalysisCase (CheckedCase path caseId model issues) = do
  rows <- mapM (\issue -> do
...
runAnalysisCase (CheckedCase path caseId model issues) = do
  ...
  pure (encodeJson (JObj [("format",JStr "yuho.case-result/v0.1"),
    ...
    ("issues",JArr rows)]) <> "\n")
  where
    runIssue issue@(CheckedIssue (CaseAllegation item _ _ _ _) _ request) = do
...
explainAnalysisCase modelPath (CheckedCase _ caseId _ issues) = do
  parts <- mapM explainIssue issues
  pure (Text.unlines (["Analysis case: " <> tokenText caseId,
    "Three independent technical issues; no aggregate case status."] ++ parts ++
    ["No guilt, conviction, acquittal, liability or sentence was determined."]))
  where
    explainIssue (CheckedIssue (CaseAllegation item kind target role _) checked _) = do
